...
void worker(std::mutex* command_access, std::queue<std::shared_ptr<Command>>* command_queue, std::string host, int port, std::string frame) {
    ...
    // Open a file to record force data
    std::ofstream dataFile;
    dataFile.open("/home/srisadha/ros/tremor_mech/data/single_Handle_force.csv");
    ...
    while (ros::ok())
    {
		...
		dataFile<<ros::Time::now()<<','<<m.wrench[0]<<','<<m.wrench[1]<<','<<m.wrench[2]<<endl;
            }
